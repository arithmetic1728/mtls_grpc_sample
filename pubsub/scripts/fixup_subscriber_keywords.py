...
class SubscriberClientCallTransformer(cst.CSTTransformer):
    CTRL_PARAMS: Tuple[str] = ('retry', 'timeout', 'metadata')

    METHOD_TO_PARAMS: Dict[str, Tuple[str]] = {
        'create_subscription': ('name', 'topic', 'push_config', 'ack_deadline_seconds', 'retain_acked_messages', 'message_retention_duration', 'labels', 'enable_message_ordering', 'expiration_policy', 'filter', 'dead_letter_policy', 'retry_policy', ),
        ...
        'streaming_pull': ('subscription', 'stream_ack_deadline_seconds', 'ack_ids', 'modify_deadline_seconds', 'modify_deadline_ack_ids', 'client_id', ),
...
def fix_files(
    ...
    files: Sequence[str],
    *,
    transformer=SubscriberClientCallTransformer(),
):
    pyfile_gen = chain(
        (os.path.join(root, f)
         for d in dirs
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description="""Fix up source that uses the Subscriber client library.
